# Michael Dziegiel - Professional Resume

## Overview

This repository hosts my current professional resume, showcasing my experience and expertise in IT infrastructure, systems administration, cloud technologies, networking, and enterprise technology solutions.

## 📄 Resume

**Current Version:** [MichaelDzieigelResume.pdf](./MichaelDzieigelResume.pdf)

## 🚀 Live Access

This resume is hosted via GitHub Pages and accessible at:

**Primary Site:**  
https://resume.mrdtech.me

**GitHub Pages Backup:**  
https://mdziegiel.github.io/resume

## 🛠️ Core Competencies

- **Systems Administration:** Windows Server, Active Directory, Group Policy
- **Cloud Platforms:** Azure / Entra ID, Microsoft 365, Intune
- **Automation:** PowerShell scripting, automation frameworks
- **Infrastructure:** Virtualization, networking, enterprise architecture
- **Security:** Identity management, compliance, cybersecurity best practices

## 📂 Repository Contents

```text
resume/
├── MichaelDzieigelResume.pdf   # Current resume PDF
├── index.html                   # Web viewer for resume
├── favicon.png                  # Site favicon
├── CNAME                        # Custom domain configuration
└── README.md                    # Repository documentation
```

## 🔄 Version History

- **Latest Update:** April 2026
- Resume is regularly updated to reflect ongoing professional growth, certifications, and enterprise IT experience

## 📫 Contact Information

For inquiries or professional opportunities, please refer to the contact details listed in the resume or connect via:

- **GitHub:** https://github.com/mdziegiel
- **LinkedIn:** https://www.linkedin.com/in/mdziegiel
- **Portfolio:** https://mdziegiel.github.io/portfolio

## 📝 Format Notes

This resume is maintained in PDF format to ensure:

- Consistent formatting across all platforms
- ATS compatibility
- Professional presentation for recruiters and hiring managers

## 📄 License

© 2026 Michael Dziegiel. All rights reserved.

---

*Hosted via GitHub Pages + Custom Domain | Updated regularly*
